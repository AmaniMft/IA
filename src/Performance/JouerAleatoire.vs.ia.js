// tester_aleatoire_vs_minimax.js
const { performance } = require('perf_hooks');
const Plateau = require('../game/Plateau');
const { minimax } = require('../ai/MinMax');
const JouerAleatoire = require('../game/JoeurAleatoire');

// Fonction pour jouer un tour avec un joueur aléatoire ou Minimax
...
// Fonction pour une partie entre Minimax et Joueur Aléatoire
function jouerPartie(profondeur = 3) {
    ...
    while (true) {
        ...
        if (mouvements.length === 0) {
            return joueur === 1 ? 'Joueur Aléatoire' : 'Minmax'; // Le joueur bloqué perd
        }

        const algo = joueur === 1 ? 'minimax' : 'random';
...
function testerPerformances(nbParties = 1000, profondeur = 3) {
    ...
    let victoiresAleatoire = 0;
    let egalites = 0;
    let tempsTotal = 0;

    console.log(`\n--- Test : Minmax vs Joueur Aléatoire sur ${nbParties} parties ---\n`);
    ...
    for (let i = 0; i < nbParties; i++) {
        ...
        else if (resultat === 'Joueur Aléatoire') victoiresAleatoire++;
        else egalites++;
    ...
    console.log(`Victoires Joueur Aléatoire : ${victoiresAleatoire}`);
